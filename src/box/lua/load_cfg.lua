    ...
    if version ~= nil then
        ...
        if schema_needs_upgrade() then
            ...
            local msg = string.format(
                'Your schema version is %s while Tarantool %s requires schema'..
                ' version %s. Please, consider using box.schema.upgrade().',
                schema_version, tarantool_version, tarantool_version)
            log.warn(msg)
        end
    end
end
